...
export class NINService {
  ...
  /**
   * Initialize NIN service with preloaded registry for offline validation
   */
  static async initialize(): Promise<void> {
    try {
      // Check if NIN registry exists, create sample data if not
      const existingRegistry = await StorageService.getItem(this.NIN_REGISTRY_KEY);
      if (!existingRegistry) {
        await this.createSampleNINRegistry();
      }
    } catch (error) {
  ...
  private static async validateNINOnline(nin: string): Promise<NINValidationResult> {
    try {
      ...
      const offlineResult = await this.validateNINOffline(nin);
  ...
  private static async validateNINOffline(nin: string): Promise<NINValidationResult> {
    try {
      ...
      if (!registry) {
        return { valid: false, error: 'NIN registry not available offline.' };
      }
      ...
      return { valid: false, error: 'NIN not found in offline registry.' };
    } catch (error) {
  ...
  /**
   * Create sample NIN registry for demo purposes
   */
  private static async createSampleNINRegistry(): Promise<void> {
    const sampleNINs = [
    ...
    for (const voter of sampleNINs) {
      const hashedNIN = await this.hashNIN(voter.nin);
      const encryptedData = await CryptoService.encryptData(JSON.stringify({
    ...
    console.log('Sample NIN registry created with', sampleNINs.length, 'entries');
  }
